...
var index = {
	...
	html: new String(),
	//publisherId: new String("ca-app-pub-5075057333402288/2294971952"),
	//interstitialAdId: new String("ca-app-pub-5075057333402288/7922703150"),
	...
	onDeviceReady: function() {
		var weinre, weinreUrl;

		document.removeEventListener('deviceready', index.onDeviceReady, false);

		if (index.weinre.enabled) {
			//console.log('Loading weinre...');
			alert('Loading weinre');
			weinre = document.createElement('script');
			weinreUrl = index.weinre.ip + ":" + index.weinre.port;
			weinreUrl += '/target/target-script-min.js';
			weinreUrl += '#' + index.weinre.targetApp;
    ...
    requestInterstitial: function (autoshow) {
        if (window.admob) {
            index.showProgress(true);
            index.autoShowInterstitial = autoshow;
    ...
    showProgress: function (show) {
        if (show) {
            addClass(index.spinner, "animated");
            removeClass(index.progressDialog, "hidden");
        } else {
            addClass(index.progressDialog, "hidden");
            removeClass(index.spinner, "animated");
        }
    },
